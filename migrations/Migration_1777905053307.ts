...
/**
 * Generated migration: Migration_1777905053307
 * Generated at: 2026-05-04T14:30:53.307Z
 */
export class Migration_1777905053307 implements MigrationInterface {
  readonly version = "1777905053307"
  ...
  async up(connection: any): Promise<void> {
    ...
    await connection.query(`CREATE TABLE IF NOT EXISTS \`subscription\` (\`id\` BIGINT AUTO_INCREMENT, \`stripe_subscription_id\` VARCHAR(255) UNIQUE, \`plan\` VARCHAR(10), \`price\` FLOAT, \`status\` VARCHAR(50), \`current_period_start\` TIMESTAMP, \`current_period_end\` TIMESTAMP, \`free_sessions_remaining\` INT, \`user_id\` BIGINT, PRIMARY KEY (\`id\`))`)
  ...
  async dryRun(connection: any): Promise<string[]> {
    return [
      ...
      "CREATE TABLE IF NOT EXISTS \`event\` (\`id\` BIGINT AUTO_INCREMENT, \`title\` VARCHAR(255), \`description\` TEXT, \`date\` DATE, \`start_time\` VARCHAR(8), \`end_time\` VARCHAR(8), \`simulators_count\` TINYINT, \`pilots_per_simulator\` TINYINT, \`price\` FLOAT, \`vehicles\` TEXT, \`vehicle_categories\` TEXT, \`access\` VARCHAR(50), \`created_at\` TIMESTAMP, PRIMARY KEY (\`id\`))",
      ...
      "CREATE TABLE IF NOT EXISTS \`subscription\` (\`id\` BIGINT AUTO_INCREMENT, \`stripe_subscription_id\` VARCHAR(255) UNIQUE, \`plan\` VARCHAR(10), \`price\` FLOAT, \`status\` VARCHAR(50), \`current_period_start\` TIMESTAMP, \`current_period_end\` TIMESTAMP, \`free_sessions_remaining\` INT, \`user_id\` BIGINT, PRIMARY KEY (\`id\`))",
